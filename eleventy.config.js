...
module.exports = function (eleventyConfig) {
  ...
  // ---------- Відео з Google Диска ----------
  // Буває, що ролик надходить не з YouTube, а посиланням на Google Диск
  // (drive.google.com/file/d/ID/view?usp=…, open?id=ID). Такий файл можна
  // вбудувати через /preview — той самий прийом «плеєр лише після кліку».
  // Доступ до файлу має бути «Усі, хто має посилання», інакше замість
  // плеєра відвідувач побачить прохання увійти в акаунт.
  ...
  function driveFileId(value) {
    ...
    const candidate =
      ...
      "";
    return DRIVE_ID.test(candidate) ? candidate : "";
  }
  eleventyConfig.addFilter("driveId", driveFileId);
  ...
  eleventyConfig.addFilter("youtubeId", (value) => {
    ...
    if (driveFileId(raw)) return ""; // це Google Диск — ним опікується driveId
