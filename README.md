...
- [ ] **Дані**: у `src/_data/site.json` стоять плейсхолдери (пошта, ЄДРПОУ, соцмережі, імена вчителів). Впишіть реальні перед запуском. Кількість учнів (166) та контекст релокації вже актуальні.
