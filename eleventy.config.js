...
module.exports = function (eleventyConfig) {
  ...
  eleventyConfig.addCollection('teachers', (api) =>
    api
      .getFilteredByGlob('src/content/teachers/*.md')
  ...
  eleventyConfig.addCollection('documents', (api) => {
    ...
    for (const doc of all) {
      ...
      if (size < DOC_MIN_BYTES) {
        console.warn(
          `[документи] пропущено «${label}»: ${onDisk} схожий на плейсхолдер (${size} Б)`,
        )
        continue
      }
      ...
      if (doc.data.altFile) {
        ...
        let altSize = -1
        try {
          altSize = fs.statSync(altOnDisk).size
        } catch {
          /* немає файлу */
        }
        ...
        if (altSize < DOC_MIN_BYTES) {
          console.warn(
            `[документи] «${label}»: додатковий файл ${altOnDisk} відсутній або порожній — кнопку не показуємо`,
          )
  ...
  eleventyConfig.addCollection('newsArchive', (api) => {
    ...
    for (const group of groups) {
      ...
      const urlFor = (n) =>
        n === 1 ? baseUrl(group.slug) : `${baseUrl(group.slug)}storinka-${n}/`
  ...
  eleventyConfig.addFilter('dateRFC3339', (value) =>
    ...
    if (url) {
      ...
      if (host === 'youtu.be') {
        ...
        candidate =
          ...
          (url.pathname.match(/^\/(?:embed|shorts|live|v)\/([^/?#]+)/) ||
            [])[1] ||
          ''
      }
    }
  ...
  eleventyConfig.addFilter('maxLessons', (week) =>
    (week || []).reduce(
      (max, day) => Math.max(max, (day.lessons || []).length),
      0,
    ),
  )
